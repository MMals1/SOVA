...
function renderNetworkPicker(context, config) {
  const { defaultNetworkKey, networkKeys, networks, optionResolver } = config;
  ...
  networkKeys
    ...
    .forEach((networkKey) => {
      ...
      button.setAttribute(
        'data-onclick',
        `selectNetworkOption('${context}', '${networkKey}', event)`,
      );
...
export const WolfPopupTemplates = {
  renderFeedbackMounts,
  renderNetworkPickers,
};
globalThis.WolfPopupTemplates = WolfPopupTemplates;
