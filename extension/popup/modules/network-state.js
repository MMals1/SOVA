(function () {
  ...
  const NETWORKS = (WalletNetworks && typeof WalletNetworks.getNetworkConfigs === 'function')
    ...
    : {
    'eth-mainnet': {
      chain: 'ethereum', chainId: 1, label: 'Ethereum Mainnet', badge: 'Ethereum Mainnet',
      isTestnet: false,
      defaultRpcUrl: getDefaultRpcUrl('eth-mainnet', 'https://ethereum-rpc.publicnode.com'),
    },
    'eth-sepolia': {
      chain: 'ethereum', chainId: 11155111, label: 'Ethereum Sepolia', badge: 'Sepolia testnet',
      isTestnet: true,
  ...
  async function initializeNetworkState() {
    ...
    PopupState.selectedChain = stored.selectedChain === DEFAULT_CHAIN_KEY ? DEFAULT_CHAIN_KEY : DEFAULT_CHAIN_KEY;
    PopupState.selectedNetwork = NETWORKS[stored.selectedNetwork] ? stored.selectedNetwork : DEFAULT_NETWORK_KEY;
    PopupState.rpcByNetwork = (stored.rpcByNetwork && typeof stored.rpcByNetwork === 'object')
      ? { ...stored.rpcByNetwork }
      : {};
  ...
  async function setNetwork(networkKey) {
    ...
    const prevAddress = (typeof globalThis.getAutoRefreshAddress === 'function')
    ...
    const isVisible = typeof globalThis.isWalletScreenVisible === 'function'
      && globalThis.isWalletScreenVisible();
    if (isVisible) {
  ...
  function _readRpcChoice() {
    ...
    return { ok: true, useDefault, url: useDefault ? null : customUrl, networkKey: PopupState.selectedNetwork };
  }
  ...
  globalThis.WolfPopupNetworkState = {
    NETWORKS, DEFAULT_NETWORK_KEY, DEFAULT_CHAIN_KEY, NETWORK_PICKER_OPTIONS, ALLOWED_RPC_HOSTS,
    getDefaultRpcUrl, initializeNetworkState, getRpcUrlForNetwork, getCurrentNetworkMeta,
    getNativeAssetSymbol, getMainnetSendGuardKey, getNetworkPickerOption, applyNetworkPickerState,
