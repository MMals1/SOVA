(function () {
  ...
  function assertModulesLoaded() {
    ...
    for (const [name, methods] of Object.entries(requirements)) {
      ...
      if (!mod) throw new Error(`Required module not loaded: ${name}. Check popup.html script order.`);
      for (const m of methods) {
        if (typeof mod[m] !== 'function') throw new Error(`${name}.${m} is missing or not a function`);
      }
    }
  }
  ...
  async function init() {
    ...
    try {
      ...
      PopupState.provider = typeof globalThis.getOrCreatePopupProvider === 'function'
      ...
      PopupState.activeAccountIndex = (activeAccount != null && activeAccount < accounts.length) ? activeAccount : 0;

      const current = accounts[PopupState.activeAccountIndex];
      if (!current?.address) {
        console.error('[popup] stored account has no address', { idx: PopupState.activeAccountIndex, total: accounts.length });
        _showScreen('screen-setup');
        return;
      }
      ...
      const goToUnlockFor = (acctName, acctAddress, statusText) => {
        ...
        document.getElementById('unlock-address').textContent = `${acctName} · ${_shortAddr(acctAddress)}`;
      ...
      if (!unlocked || expired) {
        ...
        const ensureFn = typeof globalThis.ensureActiveAccountInSW === 'function'
