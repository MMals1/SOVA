...
const _loadWalletScreen = (...a) =>
  typeof globalThis.loadWalletScreen === 'function' ? globalThis.loadWalletScreen(...a) : undefined;
...
async function unlockWallet() {
  ...
  if (!password) {
    _showError('unlock', 'Введите пароль');
    return;
  }
  ...
  if (!acct?.address) {
    console.error('[popup] active account missing after unlock', {
      activeAccountIndex: PopupState.activeAccountIndex,
  ...
  if (bus) bus.emit(bus.Events.WALLET_UNLOCKED, { address: acct.address });
}

// ── lockWallet ────────────────────────────────────────────────────────
async function lockWallet() {
  await _sendToSW({ type: MessageType.LOCK });
  const bus = globalThis.WolfPopupEventBus;
  if (bus) bus.emit(bus.Events.WALLET_LOCKED);
  ...
  document.getElementById('unlock-address').textContent = address
    ? `${name} · ${_shortAddr(address)}`
    : '';
  document.getElementById('unlock-password').value = '';
  _clearMessages('unlock');
...
async function isActiveAccountUnlocked(expectedAddress) {
  ...
  const result = await _sendToSW({ type: MessageType.GET_WALLET_ADDRESS });
...
async function ensureActiveAccountInSW(expectedAddress, accountIndex) {
  ...
  const activated = await _sendToSW({ type: MessageType.ACTIVATE_ACCOUNT, accountIndex });
...
export const WolfPopupUnlockFlow = {
  unlockWallet,
  lockWallet,
  isActiveAccountUnlocked,
  ensureActiveAccountInSW,
...
globalThis.WolfPopupUnlockFlow = WolfPopupUnlockFlow;
