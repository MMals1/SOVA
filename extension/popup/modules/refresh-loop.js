(function () {
  ...
  const _formatAmount = (...a) => typeof globalThis.formatAmount === 'function' ? globalThis.formatAmount(...a) : '0';
  const _getAccountsCached = (...a) => typeof globalThis.getAccountsCached === 'function' ? globalThis.getAccountsCached(...a) : [];
  const _updateNetworkBadge = () => { if (typeof globalThis.updateNetworkBadge === 'function') globalThis.updateNetworkBadge(); };
  const _switchWalletTab = (...a) => { if (typeof globalThis.switchWalletTab === 'function') globalThis.switchWalletTab(...a); };
  const _loadTokenBalances = (...a) => { const m = globalThis.WolfPopupTokenState; if (m) return m.loadTokenBalances(...a); };
  const _loadTransactions = (...a) => { const m = globalThis.WolfPopupTxHistory; if (m) return m.loadTransactions(...a); };
  const _setTxRefreshIndicator = (...a) => { const m = globalThis.WolfPopupTxHistory; if (m) m.setTxRefreshIndicator(...a); };
  ...
  function getAutoRefreshAddress() { return _autoRefreshAddress; }
  ...
  async function loadWalletScreen(address) {
    ...
    _sendToSW({ type: 'reset-lock-timer' });
    _setAvatar('wallet-avatar', address);

    const accounts = await _getAccountsCached();
    const acctName = accounts[PopupState.activeAccountIndex]?.name || `Account ${PopupState.activeAccountIndex + 1}`;
  ...
  async function loadBalance(address) {
    try {
      ...
      document.getElementById('wallet-balance').textContent =
        _formatAmount(parseFloat(ethers.formatEther(wei)));
    } catch {
      document.getElementById('wallet-balance').textContent = '—';
    }
  }
  ...
  globalThis.WolfPopupRefreshLoop = {
    loadWalletScreen,
