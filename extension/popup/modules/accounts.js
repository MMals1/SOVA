...
async function switchAccount(idx) {
  ...
  // Ledger accounts don't need SW unlock — go directly to wallet screen
  if (targetAccount.type === 'ledger') {
    _showScreen('screen-wallet');
    if (typeof globalThis.loadWalletScreen === 'function')
      globalThis.loadWalletScreen(targetAddress);
    const bus = globalThis.WolfPopupEventBus;
    if (bus) bus.emit(bus.Events.ACCOUNT_SWITCHED, { index: idx, address: targetAddress });
    return;
  }
  ...
  document.getElementById('unlock-address').textContent =
    `${targetName} · ${_shortAddr(targetAddress)}`;
...
async function addSubAccount() {
  ...
  if (!password) {
    _showError('add-account', 'Введите пароль');
    return;
  }
  ...
  const result = await _sendToSW({ type: MessageType.ADD_SUB_ACCOUNT, password });

  if (!result?.ok) {
    const msg =
      result?.error?.includes('password') || result?.error?.includes('пароль')
        ? 'Неверный пароль'
        : result?.error || 'Ошибка';
  ...
  _showScreen('screen-wallet');
  if (typeof globalThis.loadWalletScreen === 'function')
    globalThis.loadWalletScreen(result.address);
  const bus = globalThis.WolfPopupEventBus;
  if (bus) bus.emit(bus.Events.ACCOUNT_ADDED, { index: result.index, address: result.address });
}
...
export const WolfPopupAccounts = {
  getAccountsCached,
  setAccountsCache,
  toggleAccountMenu,
  renderAccountMenu,
...
globalThis.WolfPopupAccounts = WolfPopupAccounts;
