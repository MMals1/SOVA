...
const _setAccountsCache = (...a) =>
  typeof globalThis.setAccountsCache === 'function' ? globalThis.setAccountsCache(...a) : undefined;
...
async function createWallet() {
  ...
  if (!rpcChoice.ok) {
    _showError('create', rpcChoice.error);
    return;
  }
...
async function importWallet() {
  ...
  if (!mnemonic) {
    _showError('import', 'Введите мнемоническую фразу');
    return;
  }
  const pwErr = _validatePassword(password);
  if (pwErr) {
    _showError('import', pwErr);
    return;
  }
  ...
  const rpcChoice = ns._readRpcChoice();
  if (!rpcChoice.ok) {
    _showError('import', rpcChoice.error);
    return;
  }
...
export const WolfPopupWalletCreateImport = {
  createWallet,
  importWallet,
  _validatePassword,
};
globalThis.WolfPopupWalletCreateImport = WolfPopupWalletCreateImport;
