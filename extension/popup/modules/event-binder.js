...
function bindDeclarativeHandlers() {
  ...
  const bindAttribute = (attrName, eventName) => {
    const boundFlag = `_bound_${attrName}`;
    document.querySelectorAll(`[${attrName}]`).forEach((el) => {
      // Пропускаем уже обработанные элементы (при повторном вызове bindDeclarativeHandlers)
      if (el[boundFlag]) return;
      el[boundFlag] = true;
      const expr = (el.getAttribute(attrName) || '').trim();
      if (!expr) return;

      const enterGuard = expr.match(
        /^if\s*\(\s*event\.key\s*===\s*['"]Enter['"]\s*\)\s*([A-Za-z_$][\w$]*)\(\s*\)\s*$/,
      );
      if (enterGuard) {
...
export const WolfPopupEventBinder = {
  bindDeclarativeHandlers,
};
globalThis.WolfPopupEventBinder = WolfPopupEventBinder;
